...
release-nodocker:
	...
	$(eval GOBUILD := go build -ldflags '-X github.com/mivtdole/rtsp-simple-server/internal/core.version=$(VERSION)')
	rm -rf tmp && mkdir tmp
...
RUN export CGO_ENABLED=0 $${OPTS} \
	&& go build -ldflags "-X github.com/mivtdole/rtsp-simple-server/internal/core.version=$$VERSION" -o /rtsp-simple-server
